    ...
    try:
        ...
        for p in content_paras:
            for t_elem in p.iter():
                if t_elem.tag == f"{W_NS}t" and t_elem.text:
                    body_text += t_elem.text
        has_disclosure = "公开方式" in body_text and "依申请公开" in body_text
        if not has_disclosure:
            # 创建公开方式段落（黑体+仿宋混合）
            p_disc = ET.Element(f"{W_NS}p")
            pPr_disc = ET.SubElement(p_disc, f"{W_NS}pPr")
            jc_disc = ET.SubElement(pPr_disc, f"{W_NS}jc")
            jc_disc.set(f"{W_NS}val", "both")
            spacing_disc = ET.SubElement(pPr_disc, f"{W_NS}spacing")
            spacing_disc.set(f"{W_NS}line", "540")
            spacing_disc.set(f"{W_NS}lineRule", "exact")
            ind_disc = ET.SubElement(pPr_disc, f"{W_NS}ind")
            ind_disc.set(f"{W_NS}firstLine", "640")
            ind_disc.set(f"{W_NS}firstLineChars", "200")

            # "公开方式："用黑体
            r1 = ET.SubElement(p_disc, f"{W_NS}r")
            rPr1 = ET.SubElement(r1, f"{W_NS}rPr")
            rFonts1 = ET.SubElement(rPr1, f"{W_NS}rFonts")
            rFonts1.set(f"{W_NS}ascii", FONT_ASCII)
            rFonts1.set(f"{W_NS}hAnsi", FONT_ASCII)
            rFonts1.set(f"{W_NS}eastAsia", "黑体")
            sz1 = ET.SubElement(rPr1, f"{W_NS}sz")
            sz1.set(f"{W_NS}val", "28")
            szCs1 = ET.SubElement(rPr1, f"{W_NS}szCs")
            szCs1.set(f"{W_NS}val", "28")
            t1 = ET.SubElement(r1, f"{W_NS}t")
            t1.text = "公开方式："

            # "依申请公开"用仿宋
            r2 = ET.SubElement(p_disc, f"{W_NS}r")
            rPr2 = ET.SubElement(r2, f"{W_NS}rPr")
            rFonts2 = ET.SubElement(rPr2, f"{W_NS}rFonts")
            rFonts2.set(f"{W_NS}ascii", FONT_ASCII)
            rFonts2.set(f"{W_NS}hAnsi", FONT_ASCII)
            rFonts2.set(f"{W_NS}eastAsia", FONT_BODY)
            sz2 = ET.SubElement(rPr2, f"{W_NS}sz")
            sz2.set(f"{W_NS}val", "28")
            szCs2 = ET.SubElement(rPr2, f"{W_NS}szCs")
            szCs2.set(f"{W_NS}val", "28")
            t2 = ET.SubElement(r2, f"{W_NS}t")
            t2.text = "依申请公开"

            content_paras.append(p_disc)
