...
async function buildWithTemplate(doc, templatePath, outputPath) {
  ...
  // 合并 document.xml.rels：把生成文档中有但模板中没有的关系添加进去
  ...
  if (tRelsFile && cRelsFile) {
    ...
    const cEntries = [];
    let rm;
    while ((rm = relRe.exec(cRels)) !== null) cEntries.push(rm[0]);
    const missingRels = cEntries.filter(r => !tRels.includes(r));
    if (missingRels.length) {
      const closePos = tRels.indexOf("</Relationships>");
      if (closePos !== -1) {
        tZip.file("word/_rels/document.xml.rels",
          tRels.slice(0, closePos) + missingRels.join("\n") + tRels.slice(closePos));
      }
    }
  }
...
function buildDocument(data) {
  ...
  if (!hasDisclosureInChildren(children)) {
    addDisclosureParagraph(children);
  }
...
function main() {
  ...
  buildWithTemplate(doc, DEFAULT_TEMPLATE_PATH, outputPath).catch(console.error);
}
